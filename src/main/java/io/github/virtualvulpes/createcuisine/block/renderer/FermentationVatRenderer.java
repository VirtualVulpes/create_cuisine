package io.github.virtualvulpes.createcuisine.block.renderer;
...
public class FermentationVatRenderer extends SmartBlockEntityRenderer<FermentationVatBlockEntity> {
	public FermentationVatRenderer(BlockEntityRendererProvider.Context context) {
		super(context);
	}

	@Override
	protected void renderSafe(FermentationVatBlockEntity fermentationVat, float partialTicks, PoseStack ms, MultiBufferSource buffer,
							  int light, int overlay) {
		super.renderSafe(fermentationVat, partialTicks, ms, buffer, light, overlay);

		float fluidLevel = renderFluids(fermentationVat, partialTicks, ms, buffer, light, overlay);
		float level = Mth.clamp(fluidLevel - .3f, .125f, .6f);

		ms.pushPose();

		BlockPos pos = fermentationVat.getBlockPos();
		ms.translate(.5, .2f, .5);
		TransformStack.cast(ms)
				.rotateY(fermentationVat.ingredientRotation.getValue(partialTicks));

		RandomSource r = RandomSource.create(pos.hashCode());
		Vec3 baseVector = new Vec3(.125, level, 0);

		Storage<ItemVariant> inv = fermentationVat.itemCapability;
		if (inv != null) {
			int itemCount = 0;
			...
			for (ItemStack stack : stacks) {
				...
				if (fluidLevel > 0) {
					ms.translate(0,
							(Mth.sin(
									AnimationTickHolder.getRenderTime(fermentationVat.getLevel()) / 12f + anglePartition * itemCount) + 1.5f)
									* 1 / 32f,
							0);
				}
		...
		BlockState blockState = fermentationVat.getBlockState();
		...
		boolean outToFermentationVat = fermentationVat.getLevel()
				.getBlockState(fermentationVat.getBlockPos()
						.relative(direction))
				.getBlock() instanceof FermentationVatBlock;

		for (LongAttached<ItemStack> LongAttached : fermentationVat.visualizedOutputItems) {
			float progress = 1 - (LongAttached.getFirst() - partialTicks) / FermentationVatBlockEntity.OUTPUT_ANIMATION_TIME;

			if (!outToFermentationVat && progress > .35f)
				continue;
	...
	protected float renderFluids(FermentationVatBlockEntity fermentationVat, float partialTicks, PoseStack ms, MultiBufferSource buffer,
								 int light, int overlay) {
		SmartFluidTankBehaviour inputFluids = fermentationVat.getBehaviour(SmartFluidTankBehaviour.INPUT);
		SmartFluidTankBehaviour outputFluids = fermentationVat.getBehaviour(SmartFluidTankBehaviour.OUTPUT);
		SmartFluidTankBehaviour[] tanks = { inputFluids, outputFluids };
		float totalUnits = fermentationVat.getTotalFluidUnits(partialTicks);
		if (totalUnits < 1)
			return 0;
